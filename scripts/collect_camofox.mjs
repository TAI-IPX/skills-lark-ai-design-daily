...
async function collectViaRestOnce(handle) {
  ...
  try {
    ...
    tabId = tab.tabId || tab.id || tab.targetId;
    ...
    // Closing the final Camoufox page can leave a context that cannot create
    // another page ("Browser.newPage: window is null"). Dispose the context
    // after every account while retaining its persistent profile on disk.
...
async function collectViaRest(handle) {
  try {
    return await collectViaRestOnce(handle);
  } catch (e) {
    const message = String(e.message || e);
    const brokenContext = /window is null|Browser\.newPage|POST \/tabs .* 500/i.test(message);
    if (!brokenContext) throw e;

    console.error(`  ${handle.toLowerCase()}: stale browser context cleared; retrying once`);
    await api('DELETE', `/sessions/${USER_ID}`, { sessionKey: SESSION_KEY }).catch(() => {});
    await sleep(1000);
    return collectViaRestOnce(handle);
  }
}
