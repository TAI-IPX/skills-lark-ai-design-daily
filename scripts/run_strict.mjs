 ...
 *   7. Sends to Feishu via send_to_feishu.mjs
 */
...
function main() {
  const userId = process.env.FEISHU_USER_ID || '';
  ...
  // Step 7: send
  console.error('== strict: send to Feishu ==');
  const targetArgs = chatId ? ['--chat-id', chatId] : ['--user-id', userId];
  if (!chatId && !userId) throw new Error('missing Feishu target: set FEISHU_USER_ID or FEISHU_CHAT_ID');
  runNode(['scripts/send_to_feishu.mjs', ...targetArgs, '--as', 'bot', '--skip-validation', '--report-file', reportFile]);
